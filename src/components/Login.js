...
const[errorMessage , setErrorMessage] = useState(null);
  ...
  className='z-20 absolute top-0 overflow-x-hidden h-full w-[100%]'
   src="./imgs/Login_page.jpg" alt="img not found"  />
     ...
     <form onSubmit ={(e) => e.preventDefault()}  className='absolute text z-30 w-1/2 flex flex-col   gap-14 left-1/3 top-[9rem] ml-[22rem] '>
     <h1 className='text-[17px]  font-serif mt-[3rem]'>LOG INTO YOUR ACCOUNT</h1>
     <div className=''>
        <input ref={email} type=" text" placeholder='Email address' className='p-2  rounded w-[350px]  border-b-4 top-' />
            
        <input ref={password} type=" password" placeholder='Password' className='p-2  rounded w-[350px] bg-transparent border-b-4 mt-[2rem] flex' />
       <p className='text-red-500 font-bold'>{errorMessage }</p>
        <button   className=' rounded-lg bg-black text-white w-[350px] font-serif border-2 p-2 hover:bg-yellow-500 mt-[4rem] ' onClick={handleButtonClick}>LOG IN</button>
        </div>
